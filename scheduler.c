...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        while (recval != -1)
        {
            ...
            {
            case 1:
                printf("enqueuing Process with ID %d \n", P->ID);
        ...
        case 1:
            ...
            printf("Dequeueing Process with ID %d \n", Process->ID);
            ...
            if (pid == 0)
            {
                ...
                char SchedulerPid[GetDigitsOfInt(getpid())];
                sprintf(SchedulerPid, "%d", getpid());
                char ProcessID[GetDigitsOfInt(Process->ID)];
            ...
            if (!isEmpty(Q))
            {
                if (QTR == -1)
                {
                    ...
                    if (p->Status == NotCreated)
                    {
                        ...
                        if (pid == 0)
                        {
                            char RemainingTime[GetDigitsOfInt(p->RemainingTime)];
                            sprintf(RemainingTime, "%d", p->RemainingTime);
                            char SchedulerPid[GetDigitsOfInt(getpid())];
                            sprintf(SchedulerPid, "%d", getpid());
                            char ProcessID[GetDigitsOfInt(p->ID)];
